...
IMAGES_PATHS = sorted(list(paths.list_images(IMAGES_PATH)))
idxs = list(range(0, len(IMAGES_PATHS)))
random.seed(42)
random.shuffle(idxs)
...
# initialize the name of the directory where logs
# and output model snapshots will be stored
LOGS_AND_MODEL_DIR = "lesions_logs"

# subclass the Config class as training configuration
class LesionBoundaryConfig(Config):
    # give the configuration a recognizable name
    NAME = "lesion"

    # set the number of GPUs to use training
    # along with the number of images per GPU
    GPU_COUNT = 1
    IMAGES_PER_GPU = 1
...
# subclass the Config class as prediction configuration
...
# managing the lesion dataset
class LesionBoundaryDataset(utils.Dataset):
    def __init__(self, imagePaths, classNames, width = 1024):
        # calal the parent constructor
        super().__init__(self)

        # store the image paths and class names along with width
        # we resize images to
    ...
    def load_image(self, imageID):
        # grab the image path, load it,
        # and convert it from BGR to RGB color channel ordering
        p = self.image_info[imageID]["path"]
        image = cv2.imread(p)
        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

        # reszie the image, preserving the aspect ratio
    ...
    def load_mask(self, imageID):
        ...
        # load the annotation mask and resize it (use nearest neighbor interpolation)
        ...
        # which should be ignored and removed from the unique set
        classIDs = np.delete(classIDs, [0])

        # allocate memeory for [height, width, num of instances] array where
        # each instance effectively has its own channel
        # since there is only one lesion per image, the number of instances is 1
...
if __name__ == "__main__":
    ...
    if args["mode"] == "train":
        ...
        trainDataset = LesionBoundaryDataset(IMAGES_PATHS, CLASS_NAMES)
        trainDataset.load_lesions(trainIdxs)
        trainDataset.prepare()

        # load the validation dataset
        valDataset = LesionBoundaryDataset(IMAGES_PATHS, CLASS_NAMES)
        ...
        # initialize the model and load the COCO weights
        # so we can perform fine-tuning
        ...
        model.train(trainDataset, valDataset, epochs = 25,
            layers = "heads", learning_rate = config.LEARNING_RATE,
            augmentation = aug)

        # unfreeze the body the network and train all layers
        model.train(trainDataset, valDataset, epochs = 65,
        ...
        for i in range(0, len(r["scores"])):
            ...
            # and predicted probability from results
        ...
        # resize the image so it more easily fits on your screen
        image = imutils.resize(image, width = 512)

        # show the otuput image
